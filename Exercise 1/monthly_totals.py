...
def pivot_months_pandas(data):
    ...
    # My sum and count.
    new_sum = data.groupby(['name','month'])
    new_sum = new_sum.aggregate('sum').reset_index()
    # Adapted from the instruction.
    new_count = data.groupby(['name','month'])
    new_count = new_count.aggregate('count').reset_index()
    # Return answers.
    monthly = new_sum.pivot(index = 'name', columns = 'month', values = 'precipitation')
    counts = new_count.pivot(index = 'name', columns = 'month', values = 'precipitation')
    # Adapted from https://www.w3resource.com/pandas/dataframe/dataframe-pivot.php.
